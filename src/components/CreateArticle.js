...
export const CreateArticle = ({ setIsCreate }) => {
    ...
    const { register, handleSubmit, setValue, watch, control, setFocus, formState: { errors, isSubmitting } } = useForm({
    ...
    if (seePreview) return <PreviewArticle setSeePreview={setSeePreview} title={watch("title")} image={watch("image")} category={watch("category")} description={watch("description")} />
    return (
        <main className="pt-6 px-6 bg-gray-100 min-h-screen pb-10">
            <div className="bg-gray-50 gap-2 items-center p-6 rounded-t-lg flex">
                <button onClick={() => setIsCreate(false)} className="cursor-pointer"><ArrowLeft /></button>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-3 grid gap-5 rounded-b-lg bg-gray-50">
                ...
                <div className="flex mt-4 justify-end gap-3 text-black mb-3">
                    <Button disabled={isSubmitting} type="button" onClick={() => setIsCreate(false)} className="bg-white hover:bg-white shadow cursor-pointer text-black">Cancel</Button>
                    <Button disabled={isSubmitting} type="button" onClick={() => setSeePreview(true)} className="bg-slate-200 hover:bg-slate-300 shadow cursor-pointer text-black">Preview</Button>
